...
To clean the mci_amyloid data set, the variables were renamed and the class of variables were corrected. Here, the time_to_visit_ir:time_8 variables were changed from character to numeric variables. The cleaned mci_amyloid data set contains `r nrow(mci_amyloid_df)` observations and `r ncol(mci_amyloid_df)` variables. 

Next, we will join the mci_baseline and mci_amyloid data sets.
...
It was found that `r nrow(mci_no_overlap_df)` participants were not present in both data sets. 

In contrast, `r nrow(mci_intersection_df)` participants were present in both data sets. 
